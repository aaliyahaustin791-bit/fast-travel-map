// Fast Travel Map - Full Version
import { getContext, extension_settings } from "../../../extensions.js";
import { saveSettingsDebounced } from "../../../../script.js";
import { toastr } from "../../../../popup.js";
...
function createFloatingButton() {
    ...
    // Fixed styling with !important to override any conflicts
    btn.style.cssText = `
...
function createMapContainer() {
    ...
    container.innerHTML = `
        ...
        <div style="padding: 8px; font-size: 11px; color: #888; text-align: center; border-top: 1px solid #333;">
            Scroll: Zoom • Drag: Pan • Click: Travel • Shift+Click: Add Waypoint
        </div>
    `;
...
function createWaypoint(name, mesId) {
    ...
    return {
        ...
        discovered: mesId === -1 // Manual waypoints auto-discovered
    };
}

function isDiscovered(wp) {
    if (!settings.showFogOfWar) return true;
    if (wp.mesId === -1) return true; // Manual waypoints
    if (settings.discoveredIds.includes(wp.id)) return true;
    
    // Auto-discover within radius
...
function discoverWaypoint(wp) {
    if (!settings.discoveredIds.includes(wp.id) && !wp.discovered) {
        ...
        if (container && container.style.display === 'flex') {
            toastr.success(`Discovered: ${wp.name}`, 'New Location');
        }
    }
}
...
function onChatChanged() {
    // Reset dynamic waypoints, keep manual ones
...
async function fastTravel(waypoint) {
    if (isTraveling || !isDiscovered(waypoint)) {
        if (!isDiscovered(waypoint)) toastr.warning('Location not discovered yet!');
        return;
    }
...
function performTravel(waypoint) {
    ...
    saveSettingsDebounced();
    renderMap();
    toastr.success(`Arrived at ${waypoint.name}`, 'Travel Complete');
}
...
function renderMap() {
    ...
    settings.waypoints.forEach(wp => {
        ...
        // Connection line to next discovered
...
function handleCanvasClick(e) {
    ...
    // Add custom waypoint on shift+click
    if (e.shiftKey) {
...
console.log('[FastTravelMap] Module loaded');
